...
option = st.selectbox('Search for a movie',list_of_movies)
ID_overview = pickle.load(open("ID_overview.pkl","rb"))

selected_movie = option
index_movie = movies_list[movies_list['title'] == selected_movie].index[0]
id_selected_movie = movies_list.iloc[index_movie].id
poster = []
overview = []
poster.append(get_poster_from_API(id_selected_movie))
overview.append(ID_overview.iloc[index_movie].overview)
st.image(poster[0],width=300)
st.write(overview[0])
